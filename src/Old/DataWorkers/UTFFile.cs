...
namespace FLServer
{
    internal class UTFFile
    {
        ...
        public TreeNode LoadUTFFile(string filePath)
        {
            ...
            int stringBlockSize = Utilities.GetInt(buf, ref pos);
            pos += 4;
            ...
            TreeNode anode)
        {
            ...
            do
            {
                ...
                var data = childNode.Tag as byte[];
        ...
        private void BuildStringBlock(Dictionary<string, int> names, List<byte> stringBlock, TreeNode parent)
        {
            if (!names.ContainsKey(parent.Text))
            {
                ...
                stringBlock.AddRange(Encoding.ASCII.GetBytes(parent.Text));
                stringBlock.Add(0);
            }
            ...
            foreach (TreeNode node in parent.Nodes)
            {
                ...
                else
                {
                    if (!names.ContainsKey(node.Text))
                    {
                        ...
                        stringBlock.AddRange(Encoding.ASCII.GetBytes(node.Text));
                        stringBlock.Add(0);
                    }
                }
            }
