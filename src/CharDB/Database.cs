...
namespace FLServer.CharDB
{
	...
	static class Database
	{
			...
			using (var reader = cmd.ExecuteReader())
			{
				...
				while (reader.Read())
				{
					var a = new Account
					{
						...
						eSettings = reader.GetString(10),
						eShipState = reader.GetString(11),
						eAppearance = reader.GetString(12),
						eEquipment = reader.GetString(13),
						eCargo = reader.GetString(14),
						eRepList = reader.GetString(15),
						eVisits = reader.GetString(16),
		...
		public static bool CheckIfNameOccupied(string name)
		{
			var cmd = new SQLiteCommand(@"SELECT 1 FROM Characters Where CharName='@CharName'".Replace(@"@CharName",name),_conn);
		...
		public static bool AddAccount(string id, string charname)
		{
			using (var cmd = new SQLiteCommand(Sqladdquery,_conn))
			{
				...
				cmd.Parameters.AddWithValue("@Location", _defAcct.System);
				...
				cmd.Parameters.AddWithValue("@Settings", _defAcct.eSettings);
				cmd.Parameters.AddWithValue("@ShipState", _defAcct.eShipState);
				cmd.Parameters.AddWithValue("@Appearance", _defAcct.eAppearance);
				cmd.Parameters.AddWithValue("@Equipment", _defAcct.eEquipment);
				cmd.Parameters.AddWithValue("@Cargo", _defAcct.eCargo);
				cmd.Parameters.AddWithValue("@RepList", _defAcct.eRepList);
				cmd.Parameters.AddWithValue("@Visits", _defAcct.eVisits);
		...
		public static bool ModifyAccount(Account acc)
		{
			using (var cmd = new SQLiteCommand(SQLModQuery, _conn))
			{
				...
				cmd.Parameters.AddWithValue("@Settings", acc.eSettings);
				cmd.Parameters.AddWithValue("@ShipState", acc.eShipState);
				cmd.Parameters.AddWithValue("@Appearance", acc.eAppearance);
				cmd.Parameters.AddWithValue("@Equipment", acc.eEquipment);
				cmd.Parameters.AddWithValue("@Cargo", acc.eCargo);
				cmd.Parameters.AddWithValue("@RepList", acc.eRepList);
				cmd.Parameters.AddWithValue("@Visits", acc.eVisits);
