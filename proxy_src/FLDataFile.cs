    ...
    public class FLDataFile
    {
        public class Setting
        {
            ...
            public uint UInt(int index)
            {
                ...
                {
                    throw new Exception(String.Format("invalid int format on line '{1}' field {2}", _desc, index), e);
                }
            }
            ...
            public float Float(int index)
            {
                try
                {
                    return Convert.ToSingle(values[index], System.Globalization.NumberFormatInfo.InvariantInfo);
                }
                catch (Exception e)
                {
                    throw new Exception(String.Format("invalid float format on line '{1}' field {2}", _desc, index), e);
                }
            }
        ...
        public class Section
        {
            ...
            public Setting GetSetting(string settingName)
            {
                ...
                throw new FLDataFileException("setting " + settingName + " not found in section " + sectionName + " " + desc);
            }
        ...
        public bool DeleteSetting(string sectionName, string settingName)
        {
            ...
            foreach (Section sec in sections)
            {
                if (sec.sectionName.ToLowerInvariant() == sectionName.ToLowerInvariant())
                {
                    List<Setting> remove = new List<Setting>();
                    foreach (Setting set in sec.settings)
                    {
                        if (set.settingName.ToLowerInvariant() == settingName.ToLowerInvariant())
                        {
